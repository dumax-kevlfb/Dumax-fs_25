...
async function getChannelMessages(channel) {
  return await channel.messages.fetch({ limit: 30 });
}
...
async function updatePanel() {
  ...
  if (!panelMessage) {
    ...
    panelMessage = messages.find(m =>
      ...
      m.embeds[0]?.title?.includes("PANEL SERVEUR")
    );
...
async function updateEntreprises() {
  ...
  if (!entreprisesMessage) {
    const messages = await getChannelMessages(channel);
    entreprisesMessage = messages.find(m =>
      m.author.id === client.user.id &&
      m.embeds[0]?.title?.includes("ENTREPRISES")
    );
...
client.on("interactionCreate", async interaction => {
  ...
  if (cmd === "amende") {
    ...
    if (!AMENDES_CHANNEL_ID || AMENDES_CHANNEL_ID === "METS_TON_ID_SALON_AMENDES_ICI") {
      return interaction.reply({
        content: "❌ L’ID du salon amendes n’est pas configuré dans le code.",
        ephemeral: true
      });
    }
  ...
  if (cmd === "amende-annuler") {
    ...
    const embed = new EmbedBuilder()
      .setTitle(`❌ ━━━━━━ ANNULATION D’AMENDE N°${numero} ━━━━━━`)
      .setDescription(
    ...
    return interaction.reply({
      content: `✅ Amende N°${numero} annulée.`,
      ephemeral: true
    });
  }
